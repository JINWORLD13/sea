...
- Measured (`npm run bench` + in-browser injection): redefining the risk scan cuts pairs 250x (113x in time); the risk-pass rewrite drops subscriber notifications 499 → 1 and 40.0 → 0.7 ms (57x); batching drops set() calls 5,400 → 30 (111x). In the browser (500 tracked, 250 markers, own-ship selected, 180 msg/s injected) a batch apply takes 0.2 ms and a risk pass 0.2 ms — the data layers occupy the main thread for about 0.3 ms per second (2% of the 60 fps frame budget). Method and tables: "Measured performance" in [portfolio.en.md](portfolio.en.md).
