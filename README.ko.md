...
3. **충돌 위험 분석 (CPA/TCPA)**
   - 상대운동 2차식으로 최근접 거리·시간을 계산, 500m/6분 danger · 1,500m/12분 warning 분류
   - **거리만 보지 않음** — TCPA가 양수이고 시간 지평 안일 때만 위험. 이미 멀어지는 배는 50m 옆이라도 safe
   - **경보 플래핑 억제** — 진입선 500m / 해제선 650m 슈미트 트리거 + 강등에만 20초 유예(승격은 즉시)
   - **순수 함수로 분리** — `utils/maritimeMath.ts` + `utils/riskGrading.ts`, 항해 조우 상황 골든 케이스 36개 테스트
   - 2초 주기 스캔(본선 기준 O(n)), 제한 수역 진입 시 지오펜스 알림
...
- **해결책**:
  ...
  - **해상 수학**: COG(대지침로)·SOG(대지속력)를 속도 벡터로 변환하는 유틸리티를 만들어, 데드레커닝과 CPA 계산에 함께 사용함.
...
- **히스테리시스 상수(해제선 1.3배, 강등 유예 20초)는 실측이 아니라 판단으로 정한 값**임. 플래핑을 없애는 것과 경보가 늦게 꺼지는 것 사이의 절충이며, 실제 관제 운용 로그로 조정해야 할 부분임.
- **CPA 판정이 점(point) 기준**임. 실제 위험은 선박 길이·폭을 고려한 선체 간 거리로 봐야 하고, COLREG 조우 상황(마주침·추월·횡단)에 따라 회피 의무도 달라지지만 현재는 거리·시간만 봄.
- **성능 수치 미측정**. 최적화 전후를 FPS·프레임 타임·초당 처리 메시지로 남기는 것이 남아 있음.
