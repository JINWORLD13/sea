...
- **문제**: 글로벌 AIS 스트림에서 마커/팝업/SVG 아이콘을 과도하게 렌더링하면서 지도 스크롤 줌, 클릭 포커스, 패닝 반응이 크게 저하.
- **해결책**:
  - **뷰포트 기준 렌더링**: 현재 지도 화면(bounds) 내부 선박만 렌더링하도록 변경했습니다.
  - **렌더 상한 적용**: 한 번에 렌더하는 선박 수를 제한(`MAX_RENDERED_SHIPS`)하고 중심 근처 선박을 우선 표시하도록 정렬했습니다.
  - **아이콘 캐시 최적화**: 선박 아이콘(`DivIcon`)을 캐시하고 heading을 5도 단위로 정규화해 아이콘 재생성 비용을 줄였습니다.
