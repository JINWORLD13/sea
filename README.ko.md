...
3. 충돌 위험 분석 (CPA/TCPA) — *공식이 아니라 그 공식을 감싸는 부분이 본체*
   ...
   - 검증 — 판정 로직을 렌더링과 무관한 순수 함수로 분리하고 항해 조우 상황을 골든 케이스로 고정. Vitest 43개 통과
   - 2초 주기 CPA/TCPA 스캔(본선 기준 O(n))

4. 지오펜싱 — *구역 판정은 5줄이고, 일은 그 위의 경보 의미론에 있음*
   - 부산 제한 수역에 진입한 선박은 주황 점선 링·글로우로 즉시 구분됨. 선종 색 위에 덧대는 방식이라 선종 정보를 잃지 않음
   - 에지 트리거이지 레벨 트리거가 아님 — 밖→안으로 넘어가는 순간에만 한 번 발생함(`inRestrictedZone !== true`). 레벨 트리거면 구역에 머무는 내내 2초마다 경보가 쌓임
   - 디듀프가 에지를 삼키면 안 됨 — 피드가 나갔든 아니든 플래그는 `true`로 갱신되므로, 건너뛴 에지는 체류하는 내내 다시 오지 않음. 그래서 선박별 경보는 모든 에지마다 기록하고(20건 상한), 전역 피드에만 MMSI당 5분 디듀프를 검 — CPA 피드와 같은 창임
   - 플래그가 곧 에지 검출기의 기억 — 이걸 지우면 구역 안에 있던 배가 일제히 허위 "진입" 경보를 냄. 로컬 캐시가 이 값만 명시적 boolean으로 복원하고, "캐시 삭제"가 메모리의 선박은 일부러 건드리지 않는 이유임
   - CPA와 달리 히스테리시스가 없음 — 지오펜스는 등급이 아니라 이벤트라 떨림이 색 깜빡임이 아니라 경보 개수로 나타남. 슈미트 트리거 대신 에지 트리거·5분 피드 디듀프·20건 상한이 그 역할을 함
   - 선택과 무관하게 전 선박을 판정함(CPA는 선택한 본선이 있어야 함). `kind === "vessel"`만 대상 — 항로표지·기지국은 구역 안에 붙박이라 영원히 경보를 내게 됨
...
- 테스트: Vitest — 해상 수학·위험 등급 판정 순수 함수 + 스토어 위험 판정 패스 테스트 43개 (`npm test`)
...
│   │   └── riskPass.test.ts # 지오펜스 에지·체류 침묵·등급 이력 수명 테스트
│   ├── components/
...
- 검증: 판정 로직 전체를 렌더링과 무관한 순수 함수로 떼어내고, 항해 조우 상황(정면 대치 · 직각 횡단 · 추월 · 정지 표적 · 평행 항주 · 이격 중)을 그대로 골든 케이스로 옮겼음. 스토어의 위험 판정 패스도 함께 덮었음 — 지오펜스 진입 에지와 등급 이력의 수명은 그럴듯해 보이는 수정이 회귀를 내는 바로 그 지점이기 때문임. Vitest 43개 통과 — `npm test`.
...
- 제한 구역이 부산 해역에 하드코딩된 사각형 하나임(약 2.2 × 4.6km, 부산 해역 박스의 약 1%). 인천·싱가포르 해역에서는 지오펜싱이 통째로 동작하지 않음. 실제 어업 제한 구역은 다각형이고 권위 있는 출처에서 받아야 하며, 클라이언트에 박아 넣을 값이 아님.
- 구역 경계선을 지도에 그리지 않음. 단서는 이미 안에 들어간 배에 붙는 주황 링뿐이라, 관제사가 선이 어디를 지나는지 화면에서 알 수 없음.
- 이탈은 조용함. 이탈 경보 없이 링만 사라짐. 진입과 이탈을 비대칭으로 둔 것은 의도지만, 피드만 보면 들어온 것만 보임.
